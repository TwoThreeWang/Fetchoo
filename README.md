...
- **串行降级抓取**: HTTP → Browser → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search，智能选择最快路径，失败自动降级
- **8 种抓取策略**: HTTP、Browser（Chromium）、WebPageSnap、markdown.new、Defuddle、Jina、FxTwitter（Twitter 专用）、Brave Search（兜底搜索）
...
```
HTTP → Browser → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
...
```
Browser → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
...
```
FxTwitter → Browser → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
