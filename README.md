...
- **串行降级抓取**: HTTP → Browser → Firecrawl → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search，智能选择最快路径，失败自动降级
- **9 种抓取策略**: HTTP、Browser（Chromium）、Firecrawl（stealth 代理）、WebPageSnap、markdown.new、Defuddle、Jina、FxTwitter（Twitter 专用）、Brave Search（兜底搜索）
...
│   │   ├── firecrawl.go         # Firecrawl API（stealth 代理）
│   │   └── bravesearch.go       # Brave Search API（兜底搜索）
│   ├── headers/
...
| 变量               | 必填 | 默认值          | 说明                                             |
| ------------------ | ---- | --------------- | ------------------------------------------------ |
| `BRAVE_API_KEY`    | 否   | 空              | Brave Search API Key，用于所有抓取方式失败后的兜底搜索。免费额度 2000 次/月，[获取地址](https://api.search.brave.com) |
| `FIRECRAWL_API_KEY`| 否   | 空              | Firecrawl API Key，自带 stealth 代理绕 Cloudflare。免费额度 500 次/月，[获取地址](https://www.firecrawl.dev) |
| `TZ`               | 否   | `Asia/Shanghai` | 容器时区                                      |

> 不配置 API Key 时服务仍可正常运行，对应的降级策略会自动跳过。
...
```
HTTP → Browser → Firecrawl → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
...
```
Browser → Firecrawl → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
...
```
FxTwitter → Browser → Firecrawl → WebPageSnap → markdown.new → Defuddle → Jina → Brave Search
```
...
| 服务                                   | 用途              | 免费额度     |
| -------------------------------------- | ----------------- | ------------ |
| [Firecrawl](https://www.firecrawl.dev) | Stealth 代理抓取   | 500 次/月    |
...
| [Brave Search](https://api.search.brave.com) | 搜索快照兜底 | 2000 次/月   |
